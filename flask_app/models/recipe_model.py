...
class Recipe:
    ...
    def create(cls, data):
        query = '''
                INSERT INTO recipes(name, description, instructions, cooked_date, under_30, user_id)
    ...
    def update_one( cls, data ):
        query = '''
                ...
                under_30 = %(under_30)s, cooked_date = %(cooked_date)s, 
                user_id = %(user_id)s
                WHERE id = %(id)s;
                '''
        return connectToMySQL(DATABASE).query_db(query, data)
    ...
    def validate_recipe(data):
        ...
        if data['cooked_date'] == "":
            ...
            is_valid = False
        if len(data['name']) < 3:
            flash("Name must have at least 3 characters", "error_recipe_name")
            is_valid = False
